...
- Зафиксировано ручное разделение вендор-чанков в `vite.config.ts`; route-level `React.lazy` + `Suspense` в текущем состоянии не используется.
...
- "прикол" — фото переднем плане; "фоно" — фото на заднем плане.
